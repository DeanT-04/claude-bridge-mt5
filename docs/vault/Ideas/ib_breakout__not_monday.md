...
---
# ib_breakout__not_monday

## Hypothesis
The real timing edge of [[ib_breakout]] (beat 98-99.9% of random entries in batch 1) is concentrated in one regime: Mondays carry weekend-gap dynamics; exclude them.

## Mechanism
Same mechanism as [[ib_breakout]]; the filter `not_monday` (known pre-session) keeps only sessions where that mechanism should be strongest.

## Exact rules
Exactly [[ib_breakout]]'s rules and grid; new positions only on sessions where `not_monday` is true. Filter defined in `strategies/filters.py`.
