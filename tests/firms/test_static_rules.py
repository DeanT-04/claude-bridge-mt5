...
def phase(days, target=10_000.0, min_days=4, limit=0):
    dc, dl, ss, sd = pnl(days)
    size = np.ones(len(sd), dtype=np.int64)
    return S.sim_phase(dc, dl, ss, sd, 0, size, START, target, DAILY, MAXL, min_days, limit)
...
def test_two_phases_then_payouts() -> None:
    ...
    out = S.run_many(dc, dl, ss, sd, np.array([0]), np.ones(len(sd), dtype=np.int64), START,
                     ...
                     np.array([0, 0], dtype=np.int64), DAILY, MAXL, 0.8, 14, 365)  # fmt: skip
